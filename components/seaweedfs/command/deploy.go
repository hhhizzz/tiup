...
func newDeployCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:         "Deploy a SeaweedFS cluster for production. SSH connection will be used to deploy files, as well as creating system users for running the service.",
		SilenceUsage: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateDeployTopology(topoFile); err != nil {
				return err
			}
...
func postDeployHook(builder *task.Builder, topo cspec.Topology, _ operator.Options) {
	enableTask := task.NewBuilder(builder.Logger).
		Func("Setting service auto start on boot", func(ctx context.Context) error {
